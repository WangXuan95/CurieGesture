...
namespace BLEServiceBrowser
{
    public sealed partial class MainPage : Page
    {
        ...
        public MainPage()
        {
            ...
            NamedGuid selected = new NamedGuid() { Name = "Heart Rate Monitor", Guid = Windows.Devices.Bluetooth.GenericAttributeProfile.GattServiceUuids.HeartRate, Type = DeviceType.HeartRate };
        ...
        private void CharacteristicsList_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (index >= 0)
            {
                ...
                this.viewmodel.HandleNotificationCharateristic(true);
            }
        }
        ...
        private void BLEValueChanged(object sender, TextChangedEventArgs e)
        {
            
            string valueString = textbox.Text;
            int value = Convert.ToInt32(valueString);

            if(value>=0 && value < gestures.Length)
            {
                GestureName.Text = "Gesture: " + gestures[value];
            }
            else if(value==invalidGestureCode)
            {
                GestureName.Text = "Invalid gesture!";
            }
            else if (value == startMatchingCode)
            {
                GestureName.Text = "Start matching...";
            }
        }
